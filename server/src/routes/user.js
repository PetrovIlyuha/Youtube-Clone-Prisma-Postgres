...
async function toggleSubscribe(req, res, next) {
  if (req.user.id === req.params.userId) {
    return next({
      message: `You can't be subcriber of your own channel`,
      statusCode: 400,
    });
  }
...
async function searchUser(req, res, next) {
  ...
  if (!searchTerm) {
    return next({
      message: 'Please give a search parameter',
      statusCode: 400,
    });
  }
  ...
  for (const user of users) {
    const subscribersCount = await prisma.subscription.count({
      where: { subscribedToId: { equals: user.id } },
    });
...
async function getRecommendedChannels(req, res) {
  ...
  for (const channel of channels) {
    const subscribersCount = await prisma.subscription.count({
      where: { subscribedToId: { equals: channel.id } },
    });
    const videoCount = await prisma.video.count({
      where: {
        userId: { equals: channel.id },
      },
    });
...
async function getVideos(model, req, res) {
  ...
  const videoIds = videoRelations.map(instance => instance.videoId);
